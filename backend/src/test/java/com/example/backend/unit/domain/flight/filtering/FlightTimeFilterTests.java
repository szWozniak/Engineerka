package com.example.backend.unit.domain.flight.filtering;

import com.example.backend.common.filtering.ComparisonType;
import com.example.backend.domain.flight.filtering.FlightTimeFilter;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.function.Executable;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.ValueSource;

public class FlightTimeFilterTests {
    @ParameterizedTest
    @ValueSource(strings = { "GreaterThanOrEqual", "LesserThanOrEqual"})
    public void ShouldCreateNumberFilter(String comparisonType){
        var result = new FlightTimeFilter("attribute", "whatever", Enum.valueOf(ComparisonType.class, comparisonType));
    }

    @ParameterizedTest
    @ValueSource(strings = {"Equals", "Contains"})
    public void ShouldThrowException_WhenInvalidComparisonType(String comparisonType){
        var enumValue = Enum.valueOf(ComparisonType.class, comparisonType);
        Executable action = () -> new FlightTimeFilter("attribute", "whatever", enumValue);

        Assertions.assertThrows(IllegalArgumentException.class, action);
    }
}
